...
const itemEditar = (id:number) => {
  store.esEditar  = true
  store.idEditar  = id
  router.push( {name: 'proveedor_edit', params: {id: id}} )
...
const itemDetalle = (id:number) => {  
  ...
  if (itemTmp) {
    ...
    let tmp3 = document.getElementById("ite_dir");
    tmp3.innerHTML = itemTmp[0].direccion;    
    let tmp4 = document.getElementById("ite_ema");
    tmp4.innerHTML = itemTmp[0].email;    
    let tmp5 = document.getElementById("ite_tel");
    tmp5.innerHTML = itemTmp[0].telefono;
  }
